	...
	Picker.prototype.calcElementItemPostion = function(andGenerateItms) {
		...
		self.elementItems.forEach(function(item) {
			...
			if (andGenerateItms) {
				...
				dataItem.value = item.getAttribute('data-value') || dataItem.name;
...
(function($, document) {
	...
	var DtPicker = $.DtPicker = $.Class.extend({
		...
		getSelected: function() {
			...
			switch (type) {
				case 'datetime':
					...
					selected.name    = selected.y.name + '-' + selected.m.name + '-' + selected.d.name + ' ' + selected.h.name + ':' + selected.i.name;
					selected.valueDb = selected.y.value + selected.m.value + selected.d.value + selected.h.value + selected.i.value;
					break;
				case 'date':
					selected.value   = selected.y.value + '-' + selected.m.value + '-' + selected.d.value;
					selected.name    = selected.y.name + '-' + selected.m.name + '-' + selected.d.name;
					selected.valueDb = selected.y.value + selected.m.value + selected.d.value;
					break;
				case 'time':
					selected.value   = selected.h.value + ':' + selected.i.value;
					selected.name 	 = selected.h.name + ':' + selected.i.name;
					selected.valueDb = selected.h.value + selected.i.value;
					break;
				case 'month':
					selected.value   = selected.y.value + '-' + selected.m.value;
					selected.name   = selected.y.name + '-' + selected.m.name;
					selected.valueDb = selected.y.value + selected.m.value;
					break;
				case 'hour':
					selected.value   = selected.y.value + '-' + selected.m.value + '-' + selected.d.value + ' ' + selected.h.value;
					selected.name    = selected.y.name + '-' + selected.m.name + '-' + selected.d.name + ' ' + selected.h.name;
					selected.valueDb = selected.y.value + selected.m.value + selected.d.value + selected.h.value;
					break;
				case 'year':
					selected.value 	 = selected.y.value;
					selected.name 	 = selected.y.name;
					selected.valueDb = selected.y.value;
					break;
			}
			return selected;
		},
		...
		_create: function(options) {
			...
			self.options = options;
			var now = new Date();
			options.beginYear = options.beginYear || 2010;
